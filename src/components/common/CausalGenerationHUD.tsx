      ...
      <div className="relative bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 sm:p-7 shadow-xl shadow-slate-900/5 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className={`text-[11px] font-mono font-bold tracking-[0.2em] uppercase flex items-center gap-2 ${
                progress >= 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-blue-600 dark:text-blue-400'
              }`}>
                <span>{progress >= 100 ? 'SCM PHYSIOLOGICAL SYNTHESIS COMPLETE' : 'SCM PHYSIOLOGICAL ENGINE RUNNING'}</span>
                <span className={`text-[9px] px-2 py-0.5 rounded-md font-semibold ${
                  progress >= 100
                    ? 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800'
                    : 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800'
                }`}>
                  {progress >= 100 ? 'VERIFIED & MINTED' : 'GPU ACCELERATED'}
                </span>
              </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-right shrink-0">
            ...
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/80 px-4 py-2 rounded-2xl border border-slate-200/80 dark:border-slate-700/80">
              <span className={`font-mono text-xl font-bold tabular-nums ${progress >= 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-white'}`}>
                {progress}%
              </span>
              <span className="text-[10px] font-mono font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-widest">
                {progress >= 100 ? 'COMPLETE' : `STAGE 0${stage + 1}/04`}
              </span>
            </div>

            {progress >= 100 && onProceed && (
              <button
                onClick={onProceed}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs tracking-wide transition-all shadow-md hover:shadow-lg active:scale-95 cursor-pointer shrink-0 animate-in fade-in zoom-in-95 duration-200"
              >
                <span>Next: View CSV</span>
